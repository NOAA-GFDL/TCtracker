...
AC_LINK_IFELSE([AC_LANG_PROGRAM([@%:@include <netcdf.h>], [printf("%s",cn_inq_libvers());])],
              [NETCDF_VERSION=$(./conftest$EXEEXT)],
              [NETCDF_VESION=unknown])
CFLAGS=$_ax_c_lib_netcdf_save_CFLAGS
LDFLAGS=$_ax_c_lib_netcdf_save_LDFLAGS
LIBS=$_ax_c_lib_netcdf_save_LDFLAGS],
[NETCDF_VERSION=$_ax_c_lib_netcdf_version])dnl
AC_MSG_RESULT([$NETCDF_VERSION])dnl
AC_SUBST([NETCDF_VERSION])dnl
AC_SUBST([NETCDF_CC])dnl
AC_SUBST([NETCDF_CFLAGS])dnl
AC_SUBST([NETCDF_LDFLAGS])dnl
AC_SUBST([NETCDF_LIBS])dnl
])dnl
AS_VAR_POPDEF([ax_netcdf_lib_search])dnl
AS_VAR_POPDEF([ax_netcdf_ldflags_search])dnl
...
LDFLAGS="$NETCDF_FCLDFLAGS $_ax_fc_lib_netcdf_save_LDFLAGS"
LIBS="$NETCDF_FCLIBS $_ax_fc_lib_netcdf_save_LIBS"
AC_LINK_IFELSE([AC_LANG_PROGRAM([], [      include 'netcdf.inc'
      write (*, fmt='(a)', advance='no') trim(nf_inq_libvers())])],
...
AC_MSG_RESULT([$NETCDFF_VERSION])dnl
AC_SUBST([NETCDFF_VERSION])dnl
AC_SUBST([NETCDF_FC])dnl
AC_SUBST([NETCDF_FCFLAGS])dnl
AC_SUBST([NETCDF_FCLDFLAGS])dnl
AC_SUBST([NETCDF_FCLIBS])dnl
AC_DEFINE([HAVE_NETCDFF], [1], [Defined if you have netCDF Fortran support])dnl
])dnl
AS_VAR_POPDEF([ax_netcdf_fcflags_search])dnl
AS_VAR_POPDEF([ax_netcdf_mod_check])dnl
AS_VAR_POPDEF([ax_netcdf_lib_f90])dnl
AS_VAR_POPDEF([ax_netcdff_lib_search])dnl
AS_VAR_POPDEF([ax_netcdff_ldflags_search])dnl
])
