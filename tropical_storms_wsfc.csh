...
while ("$argv[1]" != "--")
   ...
   case -o:
      # For now, this only works on GNU Linux systems
      # Ensure analysis_dir is not a relative path
      set analysis_dir = `readlink -f "$argv[2]"`; shift argv; breaksw
   case -W:
...
&input
   ...
   do_spline  = .false.
   do_thickness = .false.
   landmask_file = '$SCRIPT_ROOT/trajectory_analysis/landsea.map'
   cmask_file = '$SCRIPT_ROOT/trajectory_analysis/imask_2'
&end
eof
