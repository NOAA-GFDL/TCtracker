...
v.box(6).nm='NI'; v.box(6).xy =[30.    0  100-30   ll];xxx=105
v.box(7).nm='SI'; v.box(7).xy =[10.  -ll  xxx-10   ll];yyy=168
v.box(8).nm='AU'; v.box(8).xy =[xxx  -ll  yyy-xxx  ll];
v.box(9).nm='SP'; v.box(9).xy =[yyy  -ll  265-yyy  ll];
...
for n=1:length(ts)
  for i=1:ts(n).num
    ...
    elseif (ts(n).lon(i) >= v.box(8).xy(1)                & ...
      ...
      ts(n).nm(i) =               8;
    elseif (ts(n).lon(i) >= v.box(9).xy(1)                & ...
	    ts(n).lon(i) <  v.box(9).xy(1)+v.box(9).xy(3) & ...
	    ts(n).lat(i) >= v.box(9).xy(2)                & ...
	    ts(n).lat(i) <  v.box(9).xy(2)+v.box(9).xy(4))
      ts(n).nm(i) =               9;
    else
      ts(n).nm(i) =               1;
    end
...
%  m=find(aaa==max(aaa)); clear aaa; %box m get the most hit, my method
  ...
  for i=1:ts(n).num
    ...
    ts(n).pdi     =ts(n).pdi     +ts(n).wind(i)^3*ts(n).delt;
    ts(n).ace     =ts(n).ace     +ts(n).wind(i)^2*ts(n).delt;
  end
